   ...
   // public Emdeleteservlet() {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String user="root";
			String pass="varuni";
			String query="delete from event_manager where ev_name=?";
			Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/eventmanagementdb",user,pass);
			PreparedStatement ps=con.prepareStatement("delete from event_manager where ev_name=?");
			ps.setString(1, name);
